...
#  [1] "#7E1700" "#8C360A" "#ac5f1f" "#A3651E" "#AD7B27" "#B99235" "#C4AB4A" "#CEC56C" "#D1DC94"
# [10] "#C9E8B5" "#B4E9CC" "#95E0D6" "#73CED5" "#54B8D0" "#3DA2C9" "#2F8CBF" "#2677B7" "#1F61AD"
# [19] "#164AA3" "#023198"
...
inst_fill <- "#8C360A" # light red (institutional databases)
corpus_fill <- "#2F8CBF" # light green (created corpora)
proc_fill <- "white"
...
draw_box <- function(
  ...
  text(x + w / 2, y + h * 0.62, label, cex = cex)
  if (nzchar(sub_label)) {
    text(x + w / 2, y + h * 0.30, sub_label, cex = sub_cex)
  }
}
...
# Sources (no extra JSTOR box)
draw_box(0.39, 0.78, 0.22, 0.07, "Elsevier API", fill = inst_fill)
draw_box(0.70, 0.78, 0.22, 0.07, "ISTEX", fill = inst_fill)
...
draw_box(
  ...
  0.09,
  "Articles-corpus",
  sub_label = glue("Top 10% articles\n{nb_articles_top_10_percent} articles (post-1960)"),
  cex = 0.85,
  sub_cex = 0.70,
  fill = corpus_fill
)

# Reduced heights at bottom
...
# Arrows: top -> sources/merge
draw_arrow(0.19, 0.90, 0.38, 0.74) # JSTOR economics journals -> Merge
draw_arrow(0.50, 0.90, 0.50, 0.85) # Scopus economics journals -> Elsevier
draw_arrow(0.81, 0.90, 0.81, 0.85) # Scopus economics journals -> ISTEX

# Arrows: sources -> merge
draw_arrow(0.50, 0.78, 0.50, 0.74) # Elsevier -> Merge
draw_arrow(0.81, 0.78, 0.62, 0.74) # ISTEX -> Merge

# Arrows: internal pipeline
...
draw_arrow(0.50, 0.42, 0.50, 0.39) # Add citation -> Compute sentence vectors
...
```{r}
#| label: fig-rv-method-diagram
#| fig-cap: "Construction and use of time-specific representative vectors."
#| echo: false
#| warning: false
#| message: false
...
digraph rv_method {
  ...
  subgraph cluster_left {
    ...
    rv1910 [label='Representative vector 1910\\n= mean(1905 ... 1915)', shape = box, style='rounded,filled', fillcolor='#8C360A'];
  ...
  subgraph cluster_right {
    ...
    s2   [label='1910_sentence_2', style='rounded,filled', fillcolor='#2F8CBF'];
    sd1  [label='...'];
    sk   [label='1910_sentence_k'];
    sd2  [label='...'];
    sn   [label='1910_sentence_N', style='rounded,filled', fillcolor='#2F8CBF'];
    top1 [label='Top 1% selected', style='rounded,filled', fillcolor='#2F8CBF'];
...
![Construction and use of time-specific representative vectors.](images/rv_method_diagram.png){#fig-rv-method-diagram width="95%"}
