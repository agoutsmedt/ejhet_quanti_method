...
Networks were constructed using bibliographic coupling, where two publications are linked if they cite common references. This approach identifies articles that draw on similar intellectual foundations, revealing thematic communities within the literature.
Raw common references counting can be misleading, as articles that cite many references overall are more likely to share citations by chance and references that are highly cited are also overall more likely to link to articles by chance. 
To weight shared references between two articles, we use the coupling strength method from [@shenRefinedMethodComputing2019]. Coupling strength weighs both the size of the reference list of articles and the number of overall citations of each reference to overweight significant co-occurrences of references.

To construct temporally coherent networks, we employed a dynamic windowing approach:

* Time window: 8 years 
* Overlapping windows: windows overlap to allow smooth temporal transitions (1980-1987, 1981-1988…)
...
Clusters (or communities) within each network were identified using the Leiden algorithm with modularity optimization [@traag_LouvainLeidenGuaranteeing_2019]. This algorithm identify groups of articles that are have strong connections to each others, but few connections to the rest of the networks (i.e., relevant sub-groups of articles across the main network).
...
For visualization and interpretation we made an alluvial graph [@fig-alluv_named_minimal] that follow these intertemporal clusters across time windows, we focused on substantive communities using two criteria:
...
Smaller or more transient clusters were coded as "grey" in visualizations to emphasize the major research streams.
...
The model was prompted to generate concise labels capturing the thematic focus of each cluster. This semi-automated approach combines quantitative bibliometric indicators with semantic interpretation to produce exploratory community labels.

For each 8-year time window, the analysis generates more detailed tables for every clusters present in that network so that we could analyze manually what characterize the clusters. Among other things we focused on:

* Biggest Nodes of the Cluster: articles ranked by number of citations including full titles, journals and authors
* Most Productive Journals: journals ranked by number of publications in the clusters, revealing the primary publication venues for each research stream
* Most Productive Authors: authors ranked by number of publications in the cluster to study concentration of research activity
* Most Cited References: references ranked by citation frequency within the cluster, with publication years, journals, first author and titles. This information identifies the foundational works that define each research cluster intellectual heritage
* Most Cited References Without IDs: references ranked by citation frequency within the cluster with unmatched IDs (typically books or working papers) ranked by frequency, capturing important influences not captured in the article-focused databases
* Most Cited Journals: journals ranked by citation frequency in cluster reference lists, showing which publication venues cluster members draw upon most heavily
