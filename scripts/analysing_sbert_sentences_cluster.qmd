...
for(cluster in list_clusters){
  ...
  if(top_sentences_filtered %>% filter(mention_rationality) %>% nrow() > 0){
    top_sentences_filtered %>% 
      ...
      kbl(escape = FALSE, col.names = c("Sentence", "Title", "Year", "Journal", "Authors", "Similarity")) %>% 
      kable_styling(bootstrap_options = c("striped","condensed"),
                    font_size = 13) %>% 
      print()
  }
  ...
  top_sentences_filtered %>% 
    slice_max(similarity, n = 15) %>% 
    select(sentence, title, publication_year, is_part_of, creator, similarity) %>% 
    kbl(escape = FALSE, col.names = c("Sentence", "Year", "Title", "Journal", "Authors", "Similarity")) %>% 
    kable_styling(bootstrap_options = c("striped","condensed"),
                  font_size = 13) %>% 
    print()
