...
data_query <- dataset %>%
  filter(
    id %in% unique(bert_df_f$id) & sentence %in% unique(bert_df_f$sentence)
  ) %>%
  collect() %>%
  distinct(id, sentence, year, .keep_all = TRUE)

bert_df_f <- merge(
  bert_df_f,
  data_query %>% select(id, sentence, year, embedding),
  by = c("id", "sentence", "year"),
  all.x = TRUE
) %>%
  as_tibble() %>%
  unique()
