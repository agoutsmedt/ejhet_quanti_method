...
wos_art <- merge(
  wos_art,
  journal_wos[, .(Code_Revue, Revue, Code_Discipline)],
  all.x = TRUE,
  by = "Code_Revue"
)
wos_art <- merge(
  wos_art,
  issue_wos[, .(IssueID, Volume, Numero)],
  all.x = TRUE,
  by = "IssueID"
)
...
relevant_art_manual_jstor_ids <- matching[
  id_match_final %in% relevant_art_manual_ids
]
...
wos_refs_filtered <- wos_refs[
  ItemID_Ref %in%
    relevant_art_manual_ids_ref |
...
# citations
wos_citations <- merge(
  wos_refs_filtered[ID_Art %in% wos_art[Code_Discipline == 119]$ID_Art],
  wos_art[, .(Annee_Bibliographique, ID_Art)],
  by = "ID_Art",
  all.x = TRUE
)
wos_citations <- merge(
  wos_citations,
  wos_art[Code_Discipline == 119, .N, Annee_Bibliographique],
  by = "Annee_Bibliographique",
  all.x = TRUE
)
wos_citations <- wos_citations %>% rename(n_art_year_tot = N)

wos_citations_top_5 <- merge(
  wos_refs_filtered[
    ID_Art %in%
      wos_art[Code_Revue %in% c("758", "13694", "4695", "9662", "13992")]$ID_Art
  ],
  wos_art[, .(Annee_Bibliographique, ID_Art)],
  by = "ID_Art",
  all.x = TRUE
...
wos_citations_top_5 <- merge(
  wos_citations_top_5,
  wos_art[
    Code_Revue %in% c("758", "13694", "4695", "9662", "13992"),
    .N,
...
citations_selected_articles <- wos_citations[,
  .N,
  .(ItemID_Ref, Annee_Bibliographique, n_art_year_tot)
]
citations_selected_articles[, share_citations := N / n_art_year_tot]
citations_selected_articles <- citations_selected_articles[
  !is.na(Annee_Bibliographique) & !is.na(share_citations)
]
...
plot_citations <- ggplot(
  citations_selected_articles,
  aes(
  ...
  scale_color_manual(
    values = c(brewer.pal(7, name = "Dark2"), brewer.pal(7, name = "Set1")),
    guide = FALSE
  ) +
  geom_label_repel(
    aes(label = label),
    data = citations_selected_articles[
      Annee_Bibliographique ==
        max(citations_selected_articles$Annee_Bibliographique) - 1
    ],
    nudge_x = 1,
    segment.color = NA
  ) +
...
ggsave(
  plot = plot_citations,
  here(data_path, "Pictures", paste0("plot_citations.png")),
  width = 32,
  height = 18,
  units = "cm"
)
...
citations_selected_articles <- complete(
  citations_selected_articles,
  ItemID_Ref,
  Annee_Bibliographique
) %>%
...
citations_selected_articles <- citations_selected_articles[
  ItemID_Ref %in% c("49148", "147083", "76538", "45628")
]

plot_citations_bounded <- ggplot(
  citations_selected_articles,
  aes(
    x = Annee_Bibliographique,
    y = share_citations,
...
>>>>>>> 8fe79014ccf4971d9c5f03a851f0f1af8c7c6e50
  theme(axis.text.x = element_text(angle = 90, vjust = 0.75)) +
...
ggsave(
  plot = plot_citations_bounded,
  here(data_path, "Pictures", paste0("plot_citations_bounded.png")),
  width = 32,
  height = 18,
  units = "cm"
)
...
plot_citations_rational <- ggplot(
  citations_selected_articles,
  aes(
    x = Annee_Bibliographique,
    y = share_citations,
...
ggsave(
  plot = plot_citations_rational,
  here(data_path, "Pictures", paste0("plot_citations_rational.png")),
  width = 32,
  height = 18,
  units = "cm"
)
...
citations_selected_articles <- wos_citations_top_5[,
  .N,
  .(ItemID_Ref, Annee_Bibliographique, n_art_year_tot)
]
citations_selected_articles[, share_citations := N / n_art_year_tot]
...
ggsave(
  plot = plot_citations,
  here(data_path, "Pictures", paste0("plot_citations_top_5.png")),
  width = 32,
  height = 18,
  units = "cm"
)
...
plot_citations_bounded_top5 <- ggplot(
  citations_selected_articles,
  aes(
    x = Annee_Bibliographique,
    y = share_citations,
...
>>>>>>> 8fe79014ccf4971d9c5f03a851f0f1af8c7c6e50
  ...
  ggsave(
  plot = plot_citations_bounded_top5,
  here(data_path, "Pictures", paste0("plot_citations_bounded_top_5.png")),
  width = 32,
  height = 18,
  units = "cm"
)
...
combined <- patchwork::wrap_plots(
  plot_citations_bounded /
    plot_citations_bounded_top5 +
    patchwork::plot_layout(axes = "collect_x")
)
ggsave(
  plot = combined,
  here(data_path, "Pictures", paste0("plot_citations_bounded_combined.png")),
  width = 32,
  height = 18,
  units = "cm"
)
...
mean_rationality_table <- merge(
  jstor_art[, .(year, id_jstor)],
  rationality_score[, .(jstor_id, mean_similarity)],
  by.x = "id_jstor",
  by.y = "jstor_id"
)
